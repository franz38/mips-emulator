use mips_emulator::{compile, get_state, init, run, state::{State, StateManager, INSTRUCTION_SIZE, REG_SIZE}};
...
fn compile_short_program() {
    ...
    assert_eq!(state[REG_SIZE+3], 0xAC0A0000);
    assert_eq!(state[REG_SIZE+4], 0x8C0B0000);
}
