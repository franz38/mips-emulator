...
fn add() {
    ...
    let state = compile_and_execute(String::from("ADD $2 $0 $1"), Some(initial_state), None);
...
fn sub() {
    ...
    let state = compile_and_execute(String::from("SUB $3 $1 $2"), Some(initial_state), None);
    assert_eq!(state.registers[0], 0);
    assert_eq!(state.registers[1], 22);
...
fn mult() {
    ...
    let state = compile_and_execute(String::from("MULT $1 $2"), Some(prev_state), None);
    ...
    let state2 = compile_and_execute(String::from("MULT $1 $2"), Some(prev_state2), None);
...
fn mflo() {
    let mut prev_state = State::blank_state();
    prev_state.lo = 77;
    let state = compile_and_execute(String::from("MFLO $1"), Some(prev_state), None);
...
fn mfhi() {
    let mut prev_state = State::blank_state();
    prev_state.hi = 22;
    let state = compile_and_execute(String::from("MFHI $1"), Some(prev_state), None);
