...
fn addi() {
    ...
    registers = compile_and_execute(String::from("ADDI $1 $0 22"), None, None).registers;
    assert_eq!(registers[0], 0);
    assert_eq!(registers[1], 22);
    registers = compile_and_execute(String::from("ADDI $1 $0 22\nADDI $2 $0 33"), None, None).registers;
    assert_eq!(registers[0], 0);
    assert_eq!(registers[1], 22);
    assert_eq!(registers[2], 33);

    registers =
        compile_and_execute(String::from("ADDI $1 $0 22"), Some(State::blank_state()), None).registers;
    assert_eq!(registers[0], 0);
    assert_eq!(registers[1], 22);
}
...
fn beq() {
    ...
    let state = compile_and_execute(String::from("BEQ $1 $2 10"), Some(prev_state), None);
    ...
    let state2 = compile_and_execute(String::from("BEQ $1 $2 10"), Some(prev_state2), None);
...
fn bne() {
    ...
    prev_state.registers[1] = 2;
    prev_state.registers[2] = 2;
    let state = compile_and_execute(String::from("BNE $1 $2 10"), Some(prev_state), None);
    assert_eq!(state.registers[1], 2);
    assert_eq!(state.registers[2], 2);
    ...
    let mut prev_state2 = State::blank_state();
    prev_state2.registers[1] = 2;
    let state2 = compile_and_execute(String::from("BNE $1 $2 10"), Some(prev_state2), None);
    assert_eq!(state2.registers[1], 2);
    assert_eq!(state2.registers[2], 0);
